          ...
          handler: async (response: RazorpayResponse) => {
            try {
              ...
              if (!response.razorpay_order_id || !response.razorpay_signature) {
                console.error("Missing verification data from Razorpay:", {
                ...
                throw new Error(
                  "Payment verification data incomplete. Please contact support with payment ID: " +
              ...
              // Determine if this is a critical error that requires support
              const isCriticalError =
                error instanceof Error &&
                (error.message.includes("verification data incomplete") ||
                  error.message.includes("contact support"));

              // If verification fails but we have payment_id, rely on webhook to mark status
              if (response.razorpay_payment_id && !isCriticalError) {
                console.log(
                  "Verification failed; relying on webhook for final status"
                );
              }

              const errorTitle = isCriticalError
                ? "Payment Issue Detected"
                : "Payment Verification Failed";
              const errorDescription = isCriticalError
                ? "Please contact support immediately with your payment details."
                : error instanceof Error
                ? error.message
