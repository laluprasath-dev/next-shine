  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        const all = {
          products: products.slice(0, 5).map((r) => mapRow(r, "products")),
          services: services.slice(0, 5).map((r) => mapRow(r, "services")),
          sim_products: simProducts.slice(0, 5).map((r) => mapRow(r, "sim_products")),
          events: events.slice(0, 5).map((r) => mapRow(r, "events")),
          sim_events: simEvents.slice(0, 5).map((r) => mapRow(r, "sim_events")),
          vehicles: vehicles.slice(0, 5).map((r) => mapRow(r, "vehicles")),
        } as const;
  ...
  const handleResultClick = (item: BaseResult) => {
    ...
    switch (item.type) {
      ...
      case "services":
        ...
        break;
      case "sim_products":
        navigate(`/sim-racing/products/${item.id}`);
        break;
      case "events":
        navigate(`/events/${item.id}`);
        break;
      case "sim_events":
        navigate(`/sim-racing/events/${item.id}`);
        break;
      case "vehicles":
