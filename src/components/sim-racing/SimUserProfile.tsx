      ...
      try {
        ...
        if (data) {
          ...
          setFormData({
            ...
            rank: data.rank || '',
            preferredPlatform: data.sim_ids?.preferredPlatform || '',
          });
          setSimIds(data.sim_ids || {});
        }
      } catch (error) {
          ...
          <Tabs defaultValue="profile" className="w-full">
                ...
                <div className="space-y-4 py-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <p className="text-sm">
                          {simUser?.sim_ids?.preferredPlatform ? 
                            platformOptions.find(p => p.value === simUser.sim_ids.preferredPlatform)?.label || 
                            simUser.sim_ids.preferredPlatform : 
                            'Not set'}
                        </p>
                      </CardContent>
                ...
                {simUser?.stats ? (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card className="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-950 dark:to-blue-950">
                      ...
                      <CardContent>
                        <p className="text-2xl font-bold">{simUser.stats.wins || 0}</p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-950 dark:to-blue-950">
                      ...
                      <CardContent>
                        <p className="text-2xl font-bold">{simUser.stats.races || 0}</p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-950 dark:to-blue-950">
                      ...
                      <CardContent>
                        <p className="text-2xl font-bold">{simUser.stats.timeTrials || 0}</p>
                      </CardContent>
                    </Card>
                  </div>
                ) : (
            ...
            <TabsContent value="sim-ids">
              {isEditing ? (
                ...
                <div className="space-y-4 py-4">
                  {simUser?.sim_ids && Object.keys(simUser.sim_ids).length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {platformOptions.map(platform => {
                        if (platform.value === 'preferredPlatform') return null;
                        
                        const id = simUser.sim_ids?.[platform.value];
                        if (!id) return null;
