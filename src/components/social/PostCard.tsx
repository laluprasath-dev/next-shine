...
function PostCardCommentInput({ value, onChange, onSend, loading }) {
  ...
  const handleEmojiSelect = (emoji) => {
    ...
    const emojiChar =
      emoji.native || emoji.emoji || emoji.unicode || emoji.char || emoji; // fallback to string
...
const PostCard = ({
  ...
  const { muted, setMuted } = useMyContext();
  ...
  const [mediaTagPositions, setMediaTagPositions] = useState([]);
      ...
      if (Array.isArray(parsedTags) && parsedTags.length > 0) {
        profilesApi.profiles
          ...
          .then(({ data, error }) => {
            ...
            if (data) setCollaboratorProfiles(data);
          });
      } else {
  ...
  const handleCommentClick = (e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    navigate(`/social/post/${id}`, { state: { scrollToComments: true } });
  };
  ...
  useEffect(() => {
    if (
      locationPath.state &&
      locationPath.state.scrollToComments &&
      commentsRef.current
    ) {
      setTimeout(() => {
          ...
          >
            {media.length > 1 ? (
                  ...
                  <SwiperSlide key={idx} className="relative">
                    {item.type === "image" ? (
                      <img
                        src={item.url}
                        alt=""
                        className="w-full h-full  min-h-[200px] md:h-[400px] object-contain"
                        loading="lazy"
                      />
                    ) : (
              ...
              <img
                src={media[0].url}
                alt="Post content"
                className="w-full object-contain h-full min-h-[200px] md:h-[400px]"
                loading="lazy"
                decoding="async"
                onError={(e) => {
              ...
              {recentComments.map((c) => (
                <li key={c.id} className="mb-1 flex items-start gap-2">
                  <div
                    ...
                    onClick={() => navigate(`/profile/${c.profile?.id}`)}
                    title="View profile"
                  >
