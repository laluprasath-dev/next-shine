...
const PostCardForHomePage = ({
  ...
  const { homeVideoMute: muted, setHomeVideoMute: setMuted } = useMyContext();
  const videoRefs = useRef<(HTMLVideoElement | null)[]>([]);
  const [isMobile, setIsMobile] = useState(false);
  const currentlyPlayingRef = useRef(null);
  const { scrollRef } = useMyContext();
  ...
  const handleEmojiSelect = (emoji) => {
    const emojiChar =
      emoji.native || emoji.emoji || emoji.unicode || emoji.char || emoji;
          ...
          >
            {media.length > 1 ? (
              ...
              <img
                src={media[0].url}
                alt="Post content"
                className="w-full object-contain h-[400px] md:h-[300px]"
                loading="lazy"
              />
            ) : (
              <div className="relative w-full  h-[400px] md:h-[300px] flex items-center">
