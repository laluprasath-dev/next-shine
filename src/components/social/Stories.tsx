...
import { socialApi, Story } from "@/integrations/supabase/modules/social";
...
const Stories = () => {
  ...
  const handleStoryCreated = async () => {
    ...
    const flattenedStories = stories.flatMap((user) =>
      user.stories.map((story) => ({
        ...story,
        full_name: user.full_name,
        avatar_url: user.avatar_url,
    ...
    const myStories = stories.find(
      (userStory) => userStory.user_id === user?.id
    );
          ...
          {stories.map((userStory) => {
            ...
            if (!isCurrentUser) {
              return (
                <StoryItem
                  key={userStory.user_id}
                  username={userStory.full_name || userStory.username || ""}
                  avatar={userStory.avatar_url || ""}
                  hasUnseenStories={userStory.hasUnseenStories}
                  onClick={() => handleStoryClick(userStory.user_id)}
                  storyBg={userStory.stories[0]?.media_url || null}
