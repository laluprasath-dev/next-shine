...
const UpdateRequestDetailsModal: React.FC<UpdateRequestDetailsModalProps> = ({
  ...
  const renderCategoryDetails = (details: any) => {
  ...
  const renderBranchDetails = (branches: any[]) => {
                ...
                <div className="space-y-3">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="flex-1">
                      <h4 className={`font-medium text-lg ${index>request?.current_data?.branches.length-1?'text-sm-red':''}`}>{branch.branchName}</h4>
  ...
  const renderRequestChanges = () => {
    ...
    switch (request.request_type) {
          ...
          <div className="md:space-y-4 flex flex-col gap-y-2">
              ...
              <Card>
                ...
                <CardContent className='max-[769px]:py-3'>
                  <div className="flex flex-wrap gap-2">
                    {changes.categories.map((category: string) => (
                      <Badge key={category} variant="outline" className={`text-sm ${!request?.current_data?.categories?.includes(category)?'bg-sm-red text-white':''}`}>{category}</Badge>
                    ))}
                  </div>
                </CardContent>
              </Card>
            )}
            {changes.category_specific_details && renderCategoryDetails(changes.category_specific_details)}
            {changes.justification && (
              <Card>
                ...
                <CardContent className='max-[769px]:py-3'>
                  <p className="text-sm text-gray-600 leading-relaxed">{changes.justification}</p>
                </CardContent>
              </Card>
            )}
          </div>
      ...
      case 'add_branch':
        return (
          <div className="space-y-4">
            {changes.branches && renderBranchDetails(changes.branches)}
            {changes.justification && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">Justification</CardTitle>
                    ...
                    if (typeof value === 'string' && (value.includes('http') || value.includes('blob:'))) {
                      return (
                        <div key={key} className="py-2 border-b border-gray-100">
                          ...
                          <div className="mt-1">
                            <DocumentLink url={String(value)} label={displayKey} />
                          </div>
                        </div>
                      );
                    }
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                <InfoRow label="Business Name" value={request.vendor_registration?.business_name || 'Unknown'} />
                ...
                {request.requester && (
                  <InfoRow label="Requested By" value={request.requester.full_name || request.requester.username || 'Unknown'} />
                )}
              </div>
            </CardContent>
          </Card>
