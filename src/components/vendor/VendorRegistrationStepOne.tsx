...
import VendorOnboardingAgreement from '@/pages/vendor/VendorOnboardAgreement';
...
const stepOneSchema = z.object({
  ...
  selected.forEach((category) => {
    ...
    if (category == "simracing") {
      ...
      categoryDetails = details?.[category.toLowerCase()]
    }
  ...
  const onSubmit = async (data: StepOneData) => {
    ...
    const filterData = data.categories.reduce((acc, item) => {
      ...
      if (matchedKey) {
        ...
        acc[categoryKey] = data.category_specific_details[matchedKey];
      }
