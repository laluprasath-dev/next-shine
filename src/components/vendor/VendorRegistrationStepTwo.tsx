...
const VendorRegistrationStepTwo = ({ onSuccess, vendorRegistration }) => {
  const { user } = useAuth();
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 md:space-y-8">
        ...
        <div className="space-y-2 md:space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button type="button" onClick={() => appendBranch({})} variant="outline" size="sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                {[
                  ...
                  <div key={idx} className={field === "contactEmail" ? "md:col-span-2" : ""}>
                    ...
                    {errors?.branches?.[index]?.[field as keyof typeof branch] && (
                      <p className="text-red-500 text-sm mt-1">
                        {errors.branches[index][field as keyof typeof branch]?.message}
                      </p>
                    )}
                  </div>
                ))}
