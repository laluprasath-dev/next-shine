...
const VendorUpdateRequestForm: React.FC<VendorUpdateRequestFormProps> = ({
  ...
  const validateCategoryDetails = (categories: string[], details: any) => {
    const errors: string[] = [];
  ...
  const validateBranchDetails = (branches: any[]) => {
    const errors: string[] = [];
  ...
  const handleSubmit = async (data: VendorUpdateRequestFormData) => {
    ...
    try {
      ...
      switch (data.request_type) {
        ...
        case 'update_details':
          requestedChanges = updateDetails;
          break;
      }

      // Add justification to requested changes
      requestedChanges.justification = data.justification;
