...
export async function sendMessage({
  ...
  const { data, error } = await supabase
    .from("messages")
    .insert([{ sender_id, receiver_id, content, message_type }])
    .single();
  return { data, error };
}
...
export function subscribeToMessages({ user1, user2, onMessage }) {
...
export async function uploadFile(file, userId, folder = "message-media") {
...
export async function deleteConversationBetweenUsers(user1, user2) {
...
export async function deleteMediaMessage(messageId, folder = "message-media") {
