...
export async function registerForEvent(eventId: string, userId: string, registrationData: any) {
  try {
      ...
      .insert([
        {
          ...
          status: 'confirmed',
          payment_amount: eventData?.fee_amount || 0,
          payment_currency: eventData?.fee_currency || 'INR',
          payment_status: eventData?.fee_amount > 0 ? 'pending' : 'not_required'
        }
      ]);
