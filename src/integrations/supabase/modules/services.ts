...
export const createService = async (
  ...
  userId: string
): Promise<{ data: ServicePost | null; error: any }> => {
  try {
...
export const getServiceByCategory=async(category:string):Promise<{data:ServicePost[]|null;error:any;}>=>{
  try {
    const {data,error} = await supabase
      .from("services")
      .select(`
...
export function parseServiceContent(servicePost: ServicePost): ServiceData {
  // Find category ID from tags
  const categoryName = servicePost.tags?.find(tag => 
    serviceCategories.some(cat => cat.name === tag)
  ) || '';
  
  const categoryId = serviceCategories.find(cat => cat.name === categoryName)?.id || '';
  
  // Parse content
  const lines = servicePost.content?.split('\n') || [];
  const title = lines[0] || '';
  
  // Extract description (content between title and first metadata line)
  ...
  // Extract metadata
  const priceMatch = servicePost.content?.match(/Price:\s*(.*)/);
  const durationMatch = servicePost.content?.match(/Duration:\s*(.*)/);
  const availabilityMatch = servicePost.content?.match(/Availability:\s*(.*)/);
  const contactMatch = servicePost.content?.match(/Contact:\s*(.*)/);
  ...
  return {
    ...
    media_urls: servicePost.media_urls
  };
}
