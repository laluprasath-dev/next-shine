...
import { deletePostAndMedia } from "@/utils/storageHelpers";
...
export const socialApi = {
  posts: {
    ...
    deletePost: async (postId: string, userId: string) => {
      ...
      if (verifyError || !post) {
        return {
          error: verifyError || new Error("Post not found"),
          data: null,
        };
      }
      ...
      // Delete the post media from storage and the post itself
      const result = await deletePostAndMedia(postId);

      if (!result.success) {
        console.warn(
          "Failed to delete post media, but post will still be deleted"
        );
      }

      return { error: result.error, data: result.success ? post : null };
    },
    ...
    getStoriesGroupedByUser: async (userId: string) => {
      ...
      rawStories.forEach((story) => {
        ...
        const storyWithViewed: Story = {
          ...
          viewed: viewed,
          overlays: (story.overlays as Story["overlays"]) || null,
  ...
  getStoryViewers: async (storyId: string) => {
    ...
    const viewerIds = viewerData.map((v) => v.viewers_id);
    ...
    const viewers: StoryViewer[] = viewerData.map((v) => {
