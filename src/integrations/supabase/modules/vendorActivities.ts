...
const calculateCategoryPerformance = async (vendorUserId: string, vendorData: any) => {
  try {
    ...
    if (vendorData.products && vendorData.products.length > 0) {
      const productIds = vendorData.products.map((p: any) => p.id);
      let shopRevenue = 0;
    ...
    if (vendorData.vehicles && vendorData.vehicles.length > 0) {
      const vehicleRevenue = vendorData.vehicles.reduce((sum: number, vehicle: any) => {
    ...
    if (vendorData.events && vendorData.events.length > 0) {
      const eventIds = vendorData.events.map((e: any) => e.id);
      let eventRevenue = 0;
    ...
    if (vendorData.simProducts && vendorData.simProducts.length > 0) {
      const simRevenue = vendorData.simProducts.reduce((sum: number, product: any) => {
...
const calculatePerformanceMetrics = async (vendorUserId: string, vendorData: any) => {
  try {
    // Get vendor's orders and revenue from order_items that match vendor's products
    const vendorProductIds = vendorData.products?.map((p: any) => p.id) || [];
    ...
    const vendorEventIds = vendorData.events?.map((e: any) => e.id) || [];
...
export const vendorActivitiesApi = {
  ...
  getAllVendorsWithActivities: async (): Promise<{ data: VendorActivityData[] | null; error: any }> => {
    try {
      ...
      const enrichedVendors = await Promise.all(
        (vendors || []).map(async (vendor) => {
          ...
          const [products, vehicles, services, events, simProducts, simEvents, simLeagues] = await Promise.all([
            ...
            supabase.from('events').select('*').eq('created_by', vendor.user_id),
  ...
  getVendorActivityById: async (vendorId: string): Promise<{ data: VendorActivityData | null; error: any }> => {
    try {
