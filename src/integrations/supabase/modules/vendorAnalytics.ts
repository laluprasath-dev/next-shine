...
export const vendorAnalyticsApi = {
  // Shop Analytics
  getShopAnalytics: async (sellerId: string): Promise<{ data: ShopAnalytics | null; error: any }> => {
    try {
...
const cities=Object.keys(majorCitiesData).reduce((acc,item)=>{
           console.log(majorCitiesData[item].orders)
          if(majorCitiesData[item].orders>0)
          {
            acc[item]=majorCitiesData[item]
            return acc
          }
          else
          {
  ...
  getEventAnalytics: async (organizerId: string): Promise<{ data: EventAnalytics | null; error: any }> => {
    try {
      ...
      const popularEvents = Object.values(eventRegistrationCounts)
      ...
      const attendanceComparison = popularEvents.slice(0, 3).map((event: any) => ({
      ...
      const recentFeedback = popularEvents.slice(0, 5).map((event: any, index: number) => ({
  ...
  getOrderTracking: async (sellerId: string): Promise<{ data: any[] | null; error: any }> => {
    try {
  ...
  // Update order status
  updateOrderStatus: async (orderId: string, status: string): Promise<{ data: any; error: any }> => {
    try {
  ...
  // Vehicle Analytics
  getVehicleAnalytics: async (sellerId: string): Promise<{ data: VehicleAnalytics | null; error: any }> => {
    try {
  ...
  getServiceAnalytics: async (vendorId: string): Promise<{ data: ServiceAnalytics | null; error: any }> => {
    try {
      ...
      const upcomingBookings = bookings?.filter(booking => 
        booking.booking_date > today && (booking.status === 'confirmed' || booking.status === 'pending')
      ).length || 0;
      ...
      Object.values(serviceStats).forEach((service: any) => {
        if (service.ratings.length > 0) {
          service.avgRating = service.ratings.reduce((sum: number, rating: number) => sum + rating, 0) / service.ratings.length;
        }
      });
      ...
      Object.values(serviceStats).forEach((service: any) => {
        allRatings.push(...service.ratings);
      });
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const dayRevenue = dayBookings
          ...
          .reduce((sum, booking) => sum + parseFloat(booking.services.price || '0'), 0);
      ...
      const categoryStats = bookings?.reduce((acc, booking) => {
        ...
        if (booking.status === 'completed') {
          const servicePrice = parseFloat(booking.services.price || '0');
          acc[category].revenue += servicePrice;
          // Mock rating
          const mockRating = Math.floor(Math.random() * 2) + 4;
          acc[category].ratings.push(mockRating);
        }
      ...
      const categoryPerformance = Object.values(categoryStats).map((cat: any) => ({
        ...
        revenue: cat.revenue,
        avgRating: cat.ratings.length > 0 
          ? cat.ratings.reduce((sum: number, rating: number) => sum + rating, 0) / cat.ratings.length 
          : 0
      }));
      ...
      const repeatCustomers = Object.values(customerBookingCounts).filter((count: any) => count > 1).length;
      ...
      const recentBookings = bookings?.slice(-10).map(booking => ({
        ...
        customer_name: booking.profiles?.full_name || booking.profiles?.username || 'Unknown Customer',
        notes: booking.notes || ''
      })) || [];
