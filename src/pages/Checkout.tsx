...
import { CourierOption } from "@/services/shippingService";
...
const Checkout = () => {
  ...
  // Use dynamic shipping if courier is selected, otherwise use default logic
  const shipping = selectedCourier
    ...
    try {
      ...
      const { data: orderData, error: orderError } = await supabase
        ...
        .insert({
          ...
          total,
          status: "pending",
          shipping_info: selectedCourier
            ? {
                courier_id: selectedCourier.courier_id,
                courier_name: selectedCourier.courier_name,
                shipping_cost: selectedCourier.rate,
      ...
      <Layout>
        <div className="container mx-auto py-10">
          ...
          <div className="mt-10 p-6 text-center border rounded-lg">
            ...
            <Button
              onClick={() =>
                navigate("/auth", { state: { redirect: "/shop/checkout" } })
              }
            >
              Sign In
            </Button>
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-4 sm:py-6 md:py-10">
        ...
        <div className="mt-4 sm:mt-6 grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-3">
          <div className="md:col-span-2">
            ...
            >
              <AccordionItem
                value="shipping"
                className="border rounded-lg mb-3 sm:mb-4"
              >
                <AccordionTrigger className="px-3 sm:px-4 py-2">
              ...
              >
                ...
                <AccordionContent className="px-3 sm:px-4 pb-3 sm:pb-4">
                  <ShippingOptions
                    cartItems={cartItems}
                    deliveryAddress={defaultAddress as any}
                    selectedCourier={selectedCourier}
                    onCourierSelect={handleCourierSelect}
                    isLoading={isLoadingShipping}
                    onLoadingChange={setIsLoadingShipping}
                  />
                </AccordionContent>
              </AccordionItem>
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {cartItems.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`flex items-center space-x-3 sm:space-x-4 p-2 sm:p-3 rounded-lg border ${
                            isOutOfStock
                        ...
                        >
                          <div
                            className="h-12 w-12 sm:h-16 sm:w-16 flex-shrink-0 rounded-md border overflow-hidden cursor-pointer hover:opacity-80 transition-opacity"
                            onClick={() =>
