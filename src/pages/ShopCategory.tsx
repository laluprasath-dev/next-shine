...
const ShopCategory = () => {
    ...
    try {
      if (category) {
        ...
        if (Object.keys(selectedFilters).length > 0) {
          ...
          const productIds = (filterData as any)?.product_ids || {};
          ...
          if (Object.keys(productIds).length > 0) {
            ...
            Object.entries(selectedFilters).forEach(([filterCategory, selectedValues]) => {
            ...
            if (filterProductIds.length > 0) {
              ...
              matchingProductIds = filterProductIds.reduce((acc, curr) => 
          ...
          if (matchingProductIds.length > 0) {
            ...
            if (selectedStatus !== "all") {
              finalProducts = finalProducts.filter(p => p.status === selectedStatus);
            }
            
            if (minPrice) {
              finalProducts = finalProducts.filter(p => p.price >= Number(minPrice));
            }
            
            if (maxPrice) {
              finalProducts = finalProducts.filter(p => p.price <= Number(maxPrice));
            }
            ...
            if (parts) {
              finalProducts = finalProducts.filter(p => p.parts === parts);
            }
            
            if (selectedSubcategory) {
              finalProducts = finalProducts.filter(p => p.subCategory === selectedSubcategory);
            }
            ...
            if (sortOption === 'price_asc') {
              ...
              finalProducts.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
            }
  ...
  const data = Object.keys(categoryData[category].subCategories);
  useEffect(() => {
    if (selectedSubcategory) {
      const data2 = categoryData[category];
      setFilterParts(data2.subCategories[selectedSubcategory]);
    }
  }, [selectedSubcategory]);
