...
const formSchema = z.object({
  ...
  event_type: z.enum(["race", "time_trial", "qualification", "practice", "championship", "bootcamp", "tournament"]).optional().default("race"),
  ...
  registration_type: z.enum(["solo", "team", "invitation_only", "open"]).optional().default("open"),
...
const SimEventEdit = () => {
  ...
  const { data: allEvents, isLoadingData } = useQuery({
  ...
  const { data: event, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      const { data, error } = await simRacingApi.events.getById(id);
      if (error) {
        console.error("Error fetching event:", error);
        return null;
      }
  ...
  useEffect(()=>{
    ...
    {
        const leagueIds=allEvents.map((item)=>{
            return item.league_id;
        })
       
       const filteredId=leagues.filter((item)=>{
...
console.log(leaguesId,leagues)
  ...
  useEffect(() => {
    if (event) {
      form.reset({
        ...
        format: event.format || "",
        platform: event.platform || "iracing",
        track: event.track || "",
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (isEditing && id) {
        await simRacingApi.events.update(id, eventData);
        toast({
          title: "Event Updated",
 ...
 {/* League */}
              ...
              <div className="flex gap-2 justify-end">
                <Button 
                  type="button" 
                  variant="outline" 
                  onClick={() => navigate("/admin/sim-events")}
                >
                  Cancel
                </Button>
