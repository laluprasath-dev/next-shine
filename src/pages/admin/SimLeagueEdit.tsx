...
const formSchema = z.object({
  ...
  max_participants: z.string().min(2,"minimum 4 participants required")
    .refine(val => val === '' || !isNaN(Number(val)), "Must be a number")
    .transform(val => val ? parseInt(val, 10) : undefined)
    .optional(),
  points_system: z.string().optional().transform(val => {
    ...
    message: "Points system must be valid format",
  }),
});
...
const SimLeagueEdit = () => {
  ...
  const { data: league, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      const { data, error } = await simRacingApi.leagues.getById(id);
      if (error) {
  ...
  useEffect(() => {
    ...
    if (league) {
      form.reset({
        ...
        points_system: league.points_system ? JSON.stringify(JSON.parse(league.points_system)) : "",
      });
    }
  }, [league, form]);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const leagueData: Partial<SimLeague> = {
        ...
        points_system: form.getValues().points_system || null,
        organizer_id:user.id
      };

      if (isEditing && id) {
        await simRacingApi.leagues.update(id, leagueData);
        toast({
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 md:space-y-8">
              ...
              <div className="flex gap-2 justify-end">
                <Button 
                  type="button" 
                  variant="outline" 
                  onClick={() => navigate("/admin/sim-leagues")}
                >
                  Cancel
                </Button>
