  ...
  return (
    <AdminLayout title="Sim Racing Leagues Management">
      <div className="flex flex-col md:flex-row justify-between gap-y-4 border md:border-0 py-4 border-gray-300 rounded-md px-4 md:px-1 w-full md:items-center mb-6">
        ...
        <Link to="/admin/sim-leagues/create">
              ...
              {filteredLeagues.length === 0 ? (
                ...
                filteredLeagues.map((league) => (
                  <TableRow key={league.id}>
                    ...
                    <TableCell className="whitespace-nowrap">{formatDate(league.start_date)}</TableCell>
                    <TableCell className="whitespace-nowrap">{formatDate(league.end_date)}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => navigate(`/admin/sim-leagues/edit/${league.id}`)}>
                            <Edit className="mr-2 h-4 w-4" /> Edit
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => navigate(`/admin/sim-leagues/events/${league.id}`)}>
                            <Calendar className="mr-2 h-4 w-4" /> Events
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => navigate(`/admin/sim-leagues/participants/${league.id}`)}>
                            <Users className="mr-2 h-4 w-4" /> Participants
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => navigate(`/admin/sim-leagues/standings/${league.id}`)}>
