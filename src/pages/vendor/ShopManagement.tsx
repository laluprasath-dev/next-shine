...
import { vendorAnalyticsApi } from "@/integrations/supabase/modules/vendorAnalytics";
    ...
    try {
      ...
      if (productToDelete.images && productToDelete.images.length > 0) {
        ...
        if (!deleteResult.success) {
          ...
          await deleteImagesFromStorage("products", productToDelete.images);
        }
      }
      ...
      if (existingPost) {
        ...
        const { error: postError } = await deletePostAndMedia(existingPost.id);
        if (postError) {
      ...
      toast({
        title: "Success",
        description:
          "Product, images, and associated social media post deleted successfully",
      });
    } catch (error) {
                    ...
                    {analytics?.outOfStockProducts?.map((product) => (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <Link to={`/vendor/shop/edit/${product.id}`}>
            ...
            <Card>
              ...
              <CardContent>
                {analytics?.orderHistory?.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {analytics?.orderHistory?.map((order) => (
            ...
            <Card>
              ...
              <CardContent>
                {analytics?.feedbacks?.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {analytics?.feedbacks?.map((feedback) => (
