...
const VendorAnalytics = () => {
  ...
  const renderEventAnalytics = () => {
    ...
    console.log(eventAnalytics)
    const count = Object.values(eventAnalytics.registrationBreakdown).reduce((count: number, item: number) => count + item, 0) as number
    return (
                  ...
                  <>
                    <ResponsiveContainer width="100%" className="h-[300px] md:h-[250px] relative md:block right-4 top-2" height={isMobile ? 300 : 200}>
                      <BarChart data={eventAnalytics.categoryPerformance}>
                        ...
                        <Tooltip
                          formatter={(value, name) => [
                            name === 'registrations' ? `${value} registrations` : formatCurrency(value),
                            name === 'registrations' ? 'Registrations' : 'Revenue'
                          ]}
                        />
            ...
            <CardContent>
              <div className="space-y-3">
                {eventAnalytics.popularEvents && eventAnalytics.popularEvents.length > 0 ? (
                  eventAnalytics.popularEvents.map((event, index) => (
            ...
            <CardContent>
              <div className="space-y-3">
                {eventAnalytics.recentRegistrations && eventAnalytics.recentRegistrations.length > 0 ? (
                  eventAnalytics.recentRegistrations.slice(0, 8).map((registration, index) => (
                            ...
                            <div key={city.city} className="flex justify-between items-center p-3 border rounded">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-muted-foreground">
                                    {city.orders} orders • {city.customers} customers
                                  </p>
                                </div>
                              </div>
                              <div className="text-right">
                            ...
                            <div key={rating} className="flex items-center gap-2">
                              ...
                              <div className="flex-1 bg-gray-200 rounded-full h-2">
                                <div
                                  ...
                                  style={{
                                    width: `${analytics.service.serviceRatings.totalRatings > 0
                                      ? (analytics.service.serviceRatings.ratingDistribution[rating as keyof typeof analytics.service.serviceRatings.ratingDistribution] / analytics.service.serviceRatings.totalRatings) * 100
                                      : 0}%`
                                  }}
                                ></div>
                              </div>
                              <span className="text-sm w-8">
                                {analytics.service.serviceRatings.ratingDistribution[rating as keyof typeof analytics.service.serviceRatings.ratingDistribution]}
                              </span>
                            </div>
                          ))}
                        </div>
